...
# 2.2.2 Using subsetted data to compare (1) whether behaviors changed with trials, and (2) whether outcome differed from null expectations   
# 2.2.2.1 Contrast between trials

# RY - fast
fit_RY_bold<-glm(cbind(unpalatable_50, palatable_50)~as.factor(exp),data = subset(df_unp50,subset = df_unp50$group2=="RY"&df_unp50$boldness_cat=="bold"),family = "binomial")
emm_RY_bold<-emmeans(fit_RY_bold,~exp)
test(pairs(emm_RY_bold))

# RY - slow
fit_RY_shy<-glm(cbind(unpalatable_50, palatable_50)~as.factor(exp),data = subset(df_unp50,subset = df_unp50$group2=="RY"&df_unp50$boldness_cat=="shy"),family = "binomial")
emm_RY_shy<-emmeans(fit_RY_shy,~exp)
test(pairs(emm_RY_shy))

# YR - fast
fit_YR_bold<-glm(cbind(unpalatable_50, palatable_50)~as.factor(exp),data = subset(df_unp50,subset = df_unp50$group2=="YR"&df_unp50$boldness_cat=="bold"),family = "binomial")
emm_YR_bold<-emmeans(fit_YR_bold,~exp)
test(pairs(emm_YR_bold))

# YR - slow
fit_YR_shy<-glm(cbind(unpalatable_50, palatable_50)~as.factor(exp),data = subset(df_unp50,subset = df_unp50$group2=="YR"&df_unp50$boldness_cat=="shy"),family = "binomial")
emm_YR_shy<-emmeans(fit_YR_shy,~exp)
test(pairs(emm_YR_shy))

# No difference between all pairwise comparisons between trials - low power could be an issue

# 2.2.2.2 whether foraging priority differed from 0.5
# colour x exploration
sum(df_unp50[c(df_unp50$exp==1&df_unp50$group2=="YR"&df_unp50$boldness_cat=="bold"),]$unpalatable_50)
sum(df_unp50[c(df_unp50$exp==1&df_unp50$group2=="YR"&df_unp50$boldness_cat=="bold"),]$palatable_50)

chisq.test(x = c(2,6))

# sex
sum(df_unp50[df_unp50$exp==10&df_unp50$sex=="f",]$unpalatable_50)
sum(df_unp50[df_unp50$exp==10&df_unp50$sex=="f",]$palatable_50)
...
# 2.3.2 Using subsetted data to compare (1) whether behaviors changed with trials, and (2) whether outcome differed from null expectations   
# 2.3.2.1 Contrast between trials

# RY - fast
fit_RY_bold<-glm(cbind(unpalatable_consumed,palatable_consumed)~as.factor(exp),data = subset(df_unpcons,subset = df_unpcons$group2=="RY"&df_unpcons$boldness_cat=="bold"),family = "binomial")
emm_RY_bold<-emmeans(fit_RY_bold,~exp)
test(pairs(emm_RY_bold))

# RY - slow
fit_RY_shy<-glm(cbind(unpalatable_consumed,palatable_consumed)~as.factor(exp),data = subset(df_unpcons,subset = df_unpcons$group2=="RY"&df_unpcons$boldness_cat=="shy"),family = "binomial")
emm_RY_shy<-emmeans(fit_RY_shy,~exp)
test(pairs(emm_RY_shy))

# YR - fast
fit_YR_bold<-glm(cbind(unpalatable_consumed,palatable_consumed)~as.factor(exp),data = subset(df_unpcons,subset = df_unpcons$group2=="YR"&df_unpcons$boldness_cat=="bold"),family = "binomial")
emm_YR_bold<-emmeans(fit_YR_bold,~exp)
test(pairs(emm_YR_bold))

# YR - slow
fit_YR_shy<-glm(cbind(unpalatable_consumed,palatable_consumed)~as.factor(exp),data = subset(df_unpcons,subset = df_unpcons$group2=="YR"&df_unpcons$boldness_cat=="shy"),family = "binomial")
emm_YR_shy<-emmeans(fit_YR_shy,~exp)
test(pairs(emm_YR_shy))

# No difference between all pairwise comparisons between trials - low power could be an issue

# 2.3.2.2 whether foraging priority differed from 0.5
# color x exploration
sum(df_unpcons[c(df_unpcons$exp==10&df_unpcons$group2=="YR"&df_unpcons$boldness_cat=="shy"),]$unpalatable_consumed)
sum(df_unpcons[c(df_unpcons$exp==10&df_unpcons$group2=="YR"&df_unpcons$boldness_cat=="shy"),]$palatable_consumed)

chisq.test(x = c(5,10))

# sex
sum(df_unpcons[df_unpcons$exp==10&df_unpcons$sex=="f",]$unpalatable_consumed)
sum(df_unpcons[df_unpcons$exp==10&df_unpcons$sex=="f",]$palatable_consumed)

chisq.test(x = c(18,27))
